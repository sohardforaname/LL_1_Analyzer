...
impl Syntax {
    //TODO:
    fn recursion_eliminate(&mut self) {
        /*let mut new_generator: HashMap<Symbol, HashSet<Production>> = HashMap::new();
        for productions in self.generators.iter() {
            new_generator.insert(productions.0.clone(), HashSet::new());
        }
        for productions in self.generators.iter() {
            for production in productions.1.iter() {
                if production.vec[0].is_end_symbol() {
                    new_generator.get_mut(productions)
                }
            }
        }
        let not_end_symbol_set = self.symbols.iter()
            .filter(|sym| { sym.is_not_end_symbol() }).cloned().collect::<HashSet<Symbol>>();
        let mut iteri = self.symbols.iter();
        while let Some(symi) = iteri.next() {
            let mut iterj = self.symbols.iter();
            while let Some(symj) = iterj.next() {
                if symi.eq(symj) {
                    break;
                }
                for production in self.generators.get(symi)
                    .unwrap().iter() {
                    let mut new_sym_vec: Vec<Symbol> = Vec::new();
                    if production.vec[0].eq(symj) {
